...
- This project does not provide any subscription information. You must prepare your own Clash subscription URL or an exported Clash YAML configuration file.
- Before running, manually configure either `CLASH_URL` or `CLASH_CONFIG_FILE` in the `.env` file. Configure only one of them, or the program will not work correctly.
...
Navigate to the project directory and edit the `.env` file to configure a Clash subscription URL or a local YAML file path.
...
Choose either `CLASH_URL` or `CLASH_CONFIG_FILE` in `.env`. To use a subscription URL:
...
Or use a local Clash YAML file. Absolute paths and paths relative to the project directory are supported:
...
New terminals will automatically load the proxy_on / proxy_off commands.

For the current terminal, run this if you need to use them immediately: source /etc/profile.d/clash.sh
